    ...
    ngOnInit() {
        ...
        this.locationService.query({filter: 'store-is-null'}).subscribe((res: Response) => {
            if (!this.store.locationId) {
                ...
                this.locationService.find(this.store.locationId).subscribe((subRes: Response) => {
    ...
    save () {
        ...
        if (this.store.id !== undefined) {
            this.storeService.update(this.store)
                .subscribe((res: Store) => this.onSaveSuccess(res), (res: Response) => this.onSaveError(res.json()));
        } else {
